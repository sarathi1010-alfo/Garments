...
# Convert markdown strings to HTML strings cleanly
def md_to_html(md_text):
    # Convert \alpha, \beta, \gamma to safe text or LaTeX entity
    md_text = md_text.replace(r"\alpha", "&alpha;")
    md_text = md_text.replace(r"\beta", "&beta;")
    md_text = md_text.replace(r"\gamma", "&gamma;")
    md_text = md_text.replace(r"\omega", "&omega;")
    md_text = md_text.replace(r"\epsilon", "&epsilon;")
    md_text = md_text.replace(r"\Delta", "&Delta;")
    md_text = md_text.replace(r"\delta", "&delta;")
    ...
    def flush_table(tbl_lines):
        if not tbl_lines:
            return ""
        # Parse table lines
        header_cols = [c.strip() for c in tbl_lines[0].split("|")[1:-1]]
        # Skip row 1 if it's separator
        data_rows = []
        for line in tbl_lines[1:]:
            if "---" in line:
                continue
        ...
        res = '<div class="overflow-x-auto my-6"><table class="w-full border-collapse border border-slate-200 text-left text-sm">\n'
        res += '<thead class="bg-slate-100">\n<tr>\n'
        for h in header_cols:
            res += f'<th class="border border-slate-200 p-2 font-semibold text-slate-800">{h}</th>\n'
        res += '</tr>\n</thead>\n<tbody>\n'
        for row in data_rows:
            res += '<tr>\n'
            for cell in row:
                res += f'<td class="border border-slate-200 p-2 text-slate-700">{cell}</td>\n'
            res += '</tr>\n'
        res += '</tbody>\n</table></div>\n'
        return res
    ...
    for line in lines:
        ...
        if sline.startswith("---"):
            if in_list:
                html_lines.append("</ul>")
                in_list = False
            html_lines.append("<hr class='my-8 border-slate-200' />")
            continue
        ...
        if sline.startswith("## "):
            if in_list:
                ...
                in_list = False
            htext = sline[3:].strip()
            html_lines.append(f"<h2>{htext}</h2>")
            continue
        ...
        if sline.startswith("### "):
            if in_list:
                ...
                in_list = False
            htext = sline[4:].strip()
            html_lines.append(f"<h3>{htext}</h3>")
            continue
...
# Guide 1 MD content
g1_md = """## Executive Summary & Technical Overview
...
- **X-Axis (Transverse Placket Feed Axis):** Drives the pneumatic folder and fabric clamp assembly longitudinally along the placket edge, maintaining constant linear velocity ($V_x = 0.35 \\text{ m/s}$) during the stitching cycle.
- **Y-Axis (Lateral Edge Tracking Axis):** Controls lateral needle bar offset, responding to real-time optical camera feedback ($500 \\text{ Hz}$ frame rate) to adjust needle position relative to fabric edge variation with $\\pm 0.02 \\text{ mm}$ repeatability.
- **Z-Axis (Vertical Needle & Presser Foot Axis):** Synchronizes needle penetration force with variable presser foot height and pressure ($15-35 \\text{ N}$), preventing elastomeric compression failure during high-speed sewing.
...
- **Panel Loading & Optical Registration:** The operator places the un-stitched sleeve panel onto the vacuum-assisted load plate. The optical camera captures panel orientation in $< 100 \\text{ ms}$.
- **Automated Folding & Tape Feeding:** The pneumatic folder engages, wrapping the contrast or self-fabric placket binder tape around the sleeve placket slit with uniform $12.0 \\text{ mm}$ extension.
- **Synchronized Stitching Cycle:** The multi-axis sewing head initiates stitch sequence at $4,500 \\text{ SPM}$. Closed-loop sensors modulate thread tension 1,000 times per second.
- **Dynamic Box-X or Bar-Tack Reinforcement:** At the placket base, the multi-axis needle bar executes a high-density 28-stitch box-tack with dynamic stitch density control to prevent tearing.
- **Integrated Thread Trimming & Stacking:** High-speed mechanical thread cutters sever needle and looper threads within $1.0 \\text{ mm}$ of the seam base before an automated robotic arm transfers the finished sleeve to the output stacker.
...
# Guide 2 MD content
g2_md = """## Executive Overview of the Karur-Erode Textile Axis
...
- **Biological Treatment:** Anaerobic and aerobic digestion reducing BOD and COD by $> 92\\%$.
- **Multi-Stage Reverse Osmosis (RO):** Concentrates salts while producing ultra-pure permeate water ($< 20 \\text{ ppm}$ TDS) re-used in fabric dyeing.
- **Multiple Effect Evaporators (MEE) & Thermal Crystallizers:** Converts remaining brine into dry salt cake (Glauber's salt), eliminating any liquid discharge into local waterways.
...
# Guide 3 MD content
g3_md = """## Executive Summary & Polymer Foundations
...
- **Choose Recycled Nylon 6,6** for extreme abrasion tactical apparel, contact sports uniforms (e.g., rugby, kabaddi), and high-temperature thermal pressing conditions.
- **Choose Bio-Based Polyamide 5,10** for premium high-flexibility activewear, yoga apparel, marathon singlets, and corporate athletic apparel where lightweight soft handfeel, superior elastic recovery, and maximum eco-branding are required.
...
print("Appended HTML formatted guides successfully!")
